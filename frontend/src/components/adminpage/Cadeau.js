...
function handleClick(id){
  ...
  console.log(input);
  Axios.put('http://localhost:5000/api/cadeau/'+id,cadeau);
}
function deleteItem(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Axios.delete("http://localhost:5000/api/cadeau/" + id);
            Swal.fire(
            'Supprimé!',
...
let i = 1;
  return (
    <>
        ...
        <h2 style={{color:"#fff"}}>Ajouter/modifier les <b>Cadeaux</b> affichés sur la page d'acceuil</h2>
            </div>
            <div class="row ">
  ...
  {nouvs.map(nouv=> 
    <tr>
      <th scope="row">{i++}</th>
      <td>{nouv.nameProd}</td>
      <td><img src={`http://localhost:5000/api/cadeau/photo/${nouv._id}`} id="imgProd"/></td>
      <td>{nouv.price}</td>
      <td>{nouv.url}</td>
      <td>
        <button id="deledit" class="btn btn-danger" onClick={() => deleteItem(nouv._id)} type="submit"><i class="far fa-edit">&nbsp;Supprimer</i></button>
